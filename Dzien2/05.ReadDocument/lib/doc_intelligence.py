...
def understand_image_with_gptv(image_path, caption):
    ...
    prompt = """**Zamień obraz na opis jego zawartości, dostosowując styl opisu do typu obrazu. Stosuj poniższe zasady klasyfikacji i opisu:**

- **Logo** → zwróć tylko słowo „logo”.
- **Wykres** → opisz typ wykresu, dane, trendy, osie i jednostki.
- **Schemat** → opisz strukturę, zależności między elementami i ich funkcje.
- **Tabela** → podaj typ danych, nagłówki kolumn i kluczowe wartości.
- **Zdjęcie** → opisz osoby, obiekty, tło i kontekst sytuacyjny.
- **Zrzut ekranu** → opisz interfejs użytkownika, widoczne funkcje i elementy.
- **Mapa** → opisz obszar geograficzny, oznaczenia, typ mapy i skalę.
- **Infografika** → podsumuj główne informacje, sposób ich wizualizacji i układ.
- **Diagram techniczny** → opisz komponenty, ich funkcje, relacje i ewentualne oznaczenia techniczne.
- **Zeskanowany dokument** → określ typ dokumentu (np. faktura, umowa) i podaj jego główne treści.  

**Zwróć wyłącznie opis — bez dodatkowych informacji, pytań ani komentarzy.**
    """
    prompt_text = (
        f"Opisz ten obrazek zgodnie z zasadami. Tytuł obrazu z dokumentu: {caption}):"
        if caption
        else "Opisz ten obrazek zgodnie z zasadami.:"
    )
